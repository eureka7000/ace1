class PaymentsController < ApplicationController
  ...
  def create
      ...
      recent_payment = Payment.where('user_id = ? and payment_status = ?', current_user.id, 'paid').order(id: :desc).first
      if recent_payment.nil? 
          @expire_date = params[:payment][:service_name].to_i.months
      else      
          recent_expire_date = recent_payment.expire_date
          
          if recent_expire_date < @expire_date
              @expire_date = params[:payment][:service_name].to_i.months
          else
              @expire_date = recent_expire_date + params[:payment][:service_name].to_i.months
          end
      end      
